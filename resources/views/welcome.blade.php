    ...
    <div class="container mx-auto mt-12 px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach($posts as $post)
                <div class="bg-white rounded-lg shadow-lg bg-green-100 overflow-hidden transform transition duration-500 hover:scale-105 hover:shadow-2xl" style="height: 400px;">
                    @if ($post->image)
                        <div class="w-full h-40 bg-cover bg-center" style="background-image: url('{{ asset('storage/' . $post->image) }}');"></div>
                    @else
                    ...
                    <div class="p-6">
                        ...
                        <p class="text-gray-700 mb-4 overflow-hidden text-ellipsis">{{ \Illuminate\Support\Str::limit(strip_tags($post->content), 100, '...') }}</p>
                        <a href="{{ route('post.show', $post->id) }}" class="text-sm text-blue-600 font-bold mt-2 inline-block">Read More</a>
